...
SERVICE_DIR="$MESH_DIR/services/caddy"
...
mesh_upload_file "$CADDY_HOST" "$SERVICE_DIR/Caddyfile" "${CADDY_DEPLOY_DIR}/Caddyfile" 0644 root root || exit 1
mesh_upload_file "$CADDY_HOST" "$SERVICE_DIR/docker-compose.caddy.yml" "${CADDY_DEPLOY_DIR}/compose.yml" 0644 root root || exit 1
