...
SERVICE_DIR="$MESH_DIR/services/web"
...
HTPASSWD_SOURCE="${WEB_HTPASSWD_SOURCE:-$MESH_DIR/configs/secrets/web.htpasswd}"
...
[ -r "$HTPASSWD_SOURCE" ] || {
    error "htpasswd file is not readable: $HTPASSWD_SOURCE (set WEB_HTPASSWD_SOURCE or create configs/secrets/web.htpasswd)"
    exit 1
}
...
cp "$SERVICE_DIR/docker-compose.web.yml" "$stage/docker-compose.yml"
cp "$SERVICE_DIR/Dockerfile" "$SERVICE_DIR/Nginx.Dockerfile" "$SERVICE_DIR/requirements.txt" "$SERVICE_DIR/nginx-cert-watch.sh" "$stage/"
cp "$SERVICE_DIR/certbot/Dockerfile" "$stage/certbot/"
cp -r "$SERVICE_DIR/app" "$stage/"
...
sed \
    ...
    "$SERVICE_DIR/nginx.conf.template" > "$stage/nginx.conf"
