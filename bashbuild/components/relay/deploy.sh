...
source "$COMPONENT_RELAY_DIR/../../lib/common.sh"
# shellcheck source=../../lib/inventory.sh
source "$COMPONENT_RELAY_DIR/../../lib/inventory.sh"
...
# shellcheck source=render_config.sh
source "$COMPONENT_RELAY_DIR/render_config.sh"
...
relay_deploy_one() {
    ...
    if [ "$(inv_stats_expose_haproxy "$inventory")" = true ] && [ "$rc" -eq 0 ]; then
        relay_apply_stats_firewall "$host" "$(inv_stats_public_port "$inventory")" "$(inv_stats_allowed_sources "$inventory")" || rc=1
    fi
...
REMOTE
    ...
    [ -n "$run_id" ] || { error "$node ($host): no managed relay backup is available"; return 1; }
    lock_id="rollback-$$"
...
if [ "${BASH_SOURCE[0]}" = "$0" ]; then
    ...
    relay_deploy_target "${2:-$MESH_DIR/configs/inventory.json}" "$1"
fi
