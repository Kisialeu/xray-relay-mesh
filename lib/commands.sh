...
mesh_command_deploy() {
    ...
    case "$component" in
        ...
        xray|nodes) target="${node:-all}"; "$MESH_DIR/deploy/deploy_nodes.sh" "$target" "$inventory" ;;
        relay) target="${node:-all}"; "$MESH_DIR/relay/deploy_mesh.sh" "$target" "$inventory" ;;
        stats) [ -z "$node" ] || { error "--node is not valid for stats"; return 1; }; "$MESH_DIR/stats/deploy_stats.sh" "$inventory" ;;
        web) [ -z "$node" ] || { error "--node is not valid for web"; return 1; }; "$MESH_DIR/web/deploy_web.sh" "$inventory" ;;
        caddy) [ -z "$node" ] || { error "--node is not valid for caddy"; return 1; }; "$MESH_DIR/caddy/deploy_caddy.sh" "$inventory" ;;
        subscriptions|subs)
            [ -z "$node" ] || { error "--node is not valid for subscriptions"; return 1; }
            "$MESH_DIR/subs/generate_subscriptions.sh" "$inventory" && "$MESH_DIR/subs/sync_subscriptions.sh" "$inventory"
            ;;
...
mesh_command_render() {
    ...
    case "$component" in
        relay)
            ...
            source "$MESH_DIR/relay/lib/render.sh"
        ...
        xray|nodes)
            ...
            source "$MESH_DIR/deploy/lib/xray_render.sh"
            # shellcheck source=../deploy/lib/hysteria_render.sh
            source "$MESH_DIR/deploy/lib/hysteria_render.sh"
            ...
            cp "$MESH_DIR/deploy/assets/entrypoint.sh" "$output/entrypoint.sh"
            cp "$MESH_DIR/deploy/assets/stats.py" "$output/stats.py"
            cp "$MESH_DIR/deploy/assets/docker-compose.xray.yml" "$output/docker-compose.yml"
